...
;
; 背景：全局配置（.gourdai/settings.json：通用设置、模型设置、会话、记忆等）按设计
; 持久化在**安装目录**内：resources/extraResources/.gourdai（后端进程 cwd 与
; -Dgourdai.home 均指向 extraResources，见 main/backend.js）。
; 但 assisted 安装器覆盖安装时，会先静默执行旧版卸载器，其默认逻辑是
; `RMDir /r $INSTDIR` —— 运行期产生的 .gourdai 随之被整体删除，随后新文件落地
; → 用户积累的配置全部丢失。
;
; 修复策略（配置仍留在安装目录，不改变存储位置语义）：
; 1. customInit   —— 安装动作开始前，把旧安装目录的 .gourdai **复制**备份到
;                     %LOCALAPPDATA% 下的暂存目录（用复制而非移动：即便用户中途
;                     取消安装，原目录配置也完好）。
; 2. customInstall —— 新文件落地后，把备份还原到新安装目录的 .gourdai，再清理暂存。
;                     该钩子在 uninstallOldVersion（删旧）之后执行，因此同目录覆盖、
;                     更换安装目录两种场景都能还原。
; 3. customRemoveFiles —— 卸载时，先调用位于安装目录内的 CLI 命令清理助手
;                     （.gourdai\bin\desktop-cli-uninstall.ps1），再删除应用文件。
;                     默认顺序是先 `RMDir /r $INSTDIR`，助手会随目录一起被删掉、
;                     导致 `gourdai` 命令的 PATH 残留无法清理，故必须在删文件前执行。
...
!macro customInstall
  ...
  ${andIf} ${FileExists} "$cfgBackupDir\*.*"
    ...
    CopyFiles /SILENT "$cfgBackupDir\*.*" "$INSTDIR\resources\extraResources\.gourdai"
    DetailPrint "Restored user config to: $INSTDIR\resources\extraResources\.gourdai"
  ${endif}
  ; 无论是否发生还原，都清理暂存目录
  ${if} $cfgBackupDir != ""
    RMDir /r "$cfgBackupDir"
...
!macro customRemoveFiles
  IfFileExists "$INSTDIR\resources\extraResources\.gourdai\bin\desktop-cli-uninstall.ps1" 0 skipCliCleanup
    DetailPrint "Removing 'gourdai' terminal command..."
