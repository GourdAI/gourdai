...
$TARGET_DIR = Join-Path $env:USERPROFILE ".gourdai"
...
if (Test-Path $TARGET_AGENTS) {
    $AGENTS_BACKUP = Join-Path $env:TEMP "gourdai_agents_backup_$(Get-Random).md"
...
# 检查 jar 文件是否存在
$JAR_FILE = Join-Path $TARGET_BIN_DIR "gourd-ai-agent.jar"
if (-not (Test-Path $JAR_FILE)) {
    Write-Host ""
    Write-Host "[Error] gourd-ai-agent.jar not found in $TARGET_BIN_DIR" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}
Write-Host "      Found gourd-ai-agent.jar" -ForegroundColor Gray
# =============================================
# [5/5] 创建启动脚本并配置 PATH
# =============================================
Write-Host ""
Write-Host "[5/5] Setting up 'gourdai' command..." -ForegroundColor Yellow
# 创建 PowerShell 启动脚本 (gourdai.ps1)
$LAUNCHER_PS1 = Join-Path $TARGET_BIN_DIR "gourdai.ps1"
$LAUNCHER_CONTENT = @'
...
if (-not (Test-Path $JarFile)) {
    Write-Host "[Error] gourd-ai-agent.jar not found" -ForegroundColor Red
...
Write-Host "      Created: gourdai.ps1" -ForegroundColor Gray
# 创建 CMD/.bat 启动脚本 (gourdai.bat)
$LAUNCHER_BAT = Join-Path $TARGET_BIN_DIR "gourdai.bat"
...
if not exist "%JAR_FILE%" (
    echo [Error] gourd-ai-agent.jar not found
...
Write-Host "      Created: gourdai.bat" -ForegroundColor Gray
# 创建 Git Bash 启动脚本 (gourdai)
$LAUNCHER_SH = Join-Path $TARGET_BIN_DIR "gourdai"
$LAUNCHER_SH_CONTENT = @'
#!/bin/bash
...
java $JAVA_OPTS -jar "$SCRIPT_DIR/gourd-ai-agent.jar" "$@"
'@
Set-Content -Path $LAUNCHER_SH -Value $LAUNCHER_SH_CONTENT -Encoding UTF8 -NoNewline
Write-Host "      Created: gourdai (for Git Bash)" -ForegroundColor Gray
# =============================================
# 配置 PATH 环境变量
# =============================================
Write-Host ""
...
Write-Host "    2. Run: 'gourdai cli' or 'gourdai web 0'"
Write-Host ""
Write-Host "  Directory structure:" -ForegroundColor Cyan
Write-Host "    $env:USERPROFILE\.gourdai\"
Write-Host "    +-- AGENTS.md       (agents config, preserved)"
Write-Host "    +-- bin/            (executables)"
Write-Host "    |   +-- gourd-ai-agent.jar"
Write-Host "    |   +-- gourdai.ps1   (PowerShell launcher)"
Write-Host "    |   +-- gourdai.bat   (CMD launcher)"
Write-Host "    |   +-- gourdai       (Git Bash launcher)"
Write-Host "    |   +-- uninstall.ps1   (uninstall script)"
Write-Host "    +-- skills/        (skill modules)"
Write-Host ""
Write-Host "  [Tip] To use gourdai immediately in current terminal:" -ForegroundColor Yellow
