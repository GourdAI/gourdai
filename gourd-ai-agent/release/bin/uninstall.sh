...
# 系统级链接
if [ -L "/usr/local/bin/gourdai" ] || [ -f "/usr/local/bin/gourdai" ]; then
    if [ "$(id -u)" -eq 0 ]; then
        rm -f /usr/local/bin/gourdai 2>/dev/null && echo "      Removed /usr/local/bin/gourdai"
    elif command -v sudo &> /dev/null; then
        sudo rm -f /usr/local/bin/gourdai 2>/dev/null && echo "      Removed /usr/local/bin/gourdai"
    fi
fi

# 用户级链接 (homebrew 或用户 bin)
if [ -L "$HOME/.local/bin/gourdai" ] || [ -f "$HOME/.local/bin/gourdai" ]; then
    rm -f "$HOME/.local/bin/gourdai" 2>/dev/null && echo "      Removed ~/.local/bin/gourdai"
fi

if [ -L "$HOME/bin/gourdai" ] || [ -f "$HOME/bin/gourdai" ]; then
    rm -f "$HOME/bin/gourdai" 2>/dev/null && echo "      Removed ~/bin/gourdai"
fi
