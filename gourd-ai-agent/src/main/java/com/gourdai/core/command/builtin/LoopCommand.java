...
public class LoopCommand implements Command {
    ...
    public String[] examples() {
        return new String[]{
            ...
            "/loop check CI status",
            "/loop cron:\"0 */5 * * * ?\" check status",
            "/loop goal:\"all tests pass\" fix auth",
            "/loop ls",
            "/loop stop <id>"
        };
    }
    ...
    private void doList(CommandContext ctx, String sessionId) {
        String workspace = ctx.getEngine().getWorkspace();
        String harnessSessions = ctx.getEngine().getHarnessSessions();
