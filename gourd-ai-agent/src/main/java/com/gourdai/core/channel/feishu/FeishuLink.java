...
public class FeishuLink implements Channel, Runnable {
    ...
    public FeishuLink(HarnessEngine engine, WebGate webGate, ChannelRoutingTable routingTable, ChannelCommandHandler commandHandler) {
        this.engine = engine;
    ...
    public void sendNotify(String text) {
        ...
        FeishuBinding binding = getActiveBinding();
        if (binding == null || binding.appId == null || binding.appSecret == null) return;
        messageExecutor.execute(() -> {
            try { sendReplyDo(binding, text); } catch (Exception e) {
                LOG.error("[Feishu] sendNotify error: {}", e.getMessage(), e);
            }
        });
    }
    ...
    public void sendReply(String sessionId, String reply, boolean isFinal) {
        ...
        messageExecutor.execute(() -> {
            try { sendReplyDo(binding, reply); } catch (Exception e) {
     ...
     * 获取当前活跃的全局绑定凭证
     */
    private FeishuBinding getActiveBinding() {
        try {
            Iterator<FeishuBinding> it = bindings.values().iterator();
    ...
    private void onImMessageReceive(ONode msg, StreamConnection conn) {
        ...
        boolean handled = commandHandler.handleMessage(getChannelName(), text, replyText -> {
            messageExecutor.execute(() -> {
                try {
                    sendReplyDo(finalBinding, replyText);
                } catch (Exception e) {
                    LOG.error("[Feishu] Command reply error: {}", e.getMessage(), e);
                }
            });
        });
        if (handled) {
            ...
            boolean consumed = commandHandler.handleMessage(getChannelName(), text, replyText -> {
                messageExecutor.execute(() -> {
                    try {
                        ...
                        LOG.error("[Feishu] Session list reply error: {}", e.getMessage(), e);
                    }
                });
            });
            if (consumed) {
        ...
        messageExecutor.execute(() -> {
            try {
                webGate.safeChatInput(finalActiveSessionId, finalProjectRoot, finalText, "Feishu");
            } catch (Exception e) {
